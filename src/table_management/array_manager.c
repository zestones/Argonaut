...
void construct_array_manager_context(int current_lexeme_lexicographic_index) {
    ...
    context.index_start_representation = NULL_VALUE;
    context.index_number_of_dimensions_representation = NULL_VALUE;
    context.index_end_representation = NULL_VALUE;
}
...
void declaration_array_start() {
    ...
    int index = insert_representation(context.index_current_type_lexicographic);
    fprintf(stdout, COLOR_RED "index: %d\n" COLOR_RESET, index);
    insert_declaration_array(context.index_current_lexeme_lexicographic, index);
